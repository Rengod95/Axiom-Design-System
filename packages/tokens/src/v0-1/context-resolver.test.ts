...
import { resolveTokenContextsV01 } from "./context-resolver.js";
import { serializeResolvedTokenManifestV02 } from "./manifest-serializer.js";

const modifiers: ResolverModifierRegistryV01 = {
  schemaVersion: "0.1",
...
const token = (
  ...
  options: {
    ...
    readonly dtcgType?: ParsedDtcgTokenV01["dtcgType"];
...
const baseTokens = (): readonly ParsedDtcgTokenV01[] => [
...
const input = (
  base: readonly ParsedDtcgTokenV01[] = baseTokens(),
  contexts: readonly TokenContextOverrideDocumentV01[] = [context("dark"), context("light")],
...
const resolve = (value: TokenResolutionInputV01) =>
  resolveTokenContextsV01(value, { domains, modifierRegistry: modifiers });
...
describe("v0.1 Token tier graph and context resolver", () => {
  it("resolves light/dark contexts deterministically through Semantic and Component aliases", () => {
    ...
    expect(serializeResolvedTokenManifestV02(reordered.manifest)).toBe(
      serializeResolvedTokenManifestV02(result.manifest),
    );
    expect(serializeResolvedTokenManifestV02(result.manifest).endsWith("\n")).toBe(true);
  });
