...
const isCanonicalStateRegistry = (value: unknown): boolean => hasKeys(value, ["schemaVersion", "states"]) && value["schemaVersion"] === "0.2" && Array.isArray(value["states"]) && isOrderedIdentifiers(value["states"].map((state) => isPlainRecord(state) ? state["id"] : "")) && value["states"].every((state) => hasKeys(state, ["id", "axis", "valueType", "applicableComponents", "usage"], ["values"]) && isIdentifier(state["id"]) && (state["axis"] === "lifecycle" || state["axis"] === "state") && (state["valueType"] === "boolean" || state["valueType"] === "enum") && isUniqueStrings(state["applicableComponents"]) && (state["applicableComponents"] as readonly string[]).every(isIdentifier) && isUniqueStrings(state["usage"]) && (state["usage"] as readonly string[]).some((usage) => usage === "appearance" || usage === "motion") && (state["usage"] as readonly string[]).every((usage) => usage === "appearance" || usage === "behavior" || usage === "motion") && (state["valueType"] === "enum" ? isUniqueStrings(state["values"], 2) && (state["values"] as readonly string[]).every(isIdentifier) : state["values"] === undefined) && (state["axis"] !== "lifecycle" || state["valueType"] === "boolean"));
...
const diagnostic = (
  ...
  ...(tokenId === undefined ? {} : { tokenId, target: tokenId }),
...
const validateReferenceValue = (
  ...
  for (const item of entries) {
    diagnostics.push(...propertyDiagnostics(input, property, path, mode, item.result.domain, undefined, isNegated, item.reference.path));
