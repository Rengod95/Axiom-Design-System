# Axiom v0.1 Foundation Reconciliation & Implementation Plan

**Date:** 2026-09-01 \
**Status:** ACTIVE PLAN \
**Normative inputs:** ADR-0001 through ADR-0003, SSOT-00 v0.3, SSOT-01 v0.3, SSOT-02 v0.3, SSOT-03 v0.2, SSOT-04 v0.1.1, SSOT-05 v0.2, Token Domain & CSS Binding Catalog v0.1 \
...
- add `@terrazzo/parser` behind `TokenParserPort`;
- pin dependency and parser configuration;
- normalize parser output into Axiom-owned plain data;
- add fixtures for all DTCG 2025.10 types;
- prohibit parser-specific instances in manifests.

### P2.2 Identity and Domain

- implement explicit `<domain>.<tier>.*` parsing;
- implement Token Domain Registry schema/data;
...
- validate Domain ↔ DTCG type;
- validate Domain numeric/range constraints;
- enforce non-negative Primitive spacing while reserving governed negation for
  margin/inset use sites;
- generate `TokenDomain`, `TokenTier`, and Token path types.

### P2.3 Tier graph

- validate allowed tier edges;
- reject Component → Primitive;
- require Component base alias to Semantic;
- emit dependency trace for every public Token.

### P2.4 Context resolver

- implement light/dark modifier registry;
- apply source precedence before alias resolution;
- enforce immutable tier/domain/type;
- enforce context completeness;
- emit byte-stable resolved manifests.
...
3. Add DTCG parser port with all-type fixtures.
4. Add context/resolved manifest schemas.
