...
ADR-0012 adds [typed/content checks and same-project source bundles](typed-bundle-evidence.md) as a stronger explicit profile. ADR-0013 adds [bounded IndexedDB and real Chromium storage evidence](browser-storage-evidence.md). Remaining I1 work includes unresolved named types and their public types, semantic obligations, token aliases, registry/library locks and version resolution, independent-copy and asset/dependency interchange, registered migration pairs/loss reports, full browser conformance and history capacity. The [implementation map](implementation-roadmap.json) preserves I2–I6 and the intended React/RN/Swift/Android initial release scope.
