...
| [Module guard](../../scripts/check-implementation.mjs) | AST-based dependency direction/public barrels, portable core/browser modules, a single pinned browser hash entry, code-generation/type-suppression boundaries and fourteen negative cases |
...
| [Reference verifier](../../scripts/verify-retirement.py) | Exact frozen 442-file restoration, 428 retired paths still absent, four explicitly approved replacement root files, legal-file preservation and eight negative cases |
...
- The local principal is supplied by the CLI's trusted process adapter. Network/API authentication, memberships, remote synchronization and collaboration are not implemented. The later [browser storage increment](browser-storage-evidence.md) shares the core and has bounded Chromium evidence; it does not imply Studio or full offline readiness.
