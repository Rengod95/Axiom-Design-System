...
test("editorial type uses authored composite measurements and family instead of a generic preview size", () => {
  ...
  assert.deepEqual(style, { fontFamily: "Geist, sans-serif", fontWeight: 600, fontSize: 48, letterSpacing: -.4, lineHeight: 1.1 });
...
test("dependency diagrams follow active overrides and retain actual composite/property edges", () => {
  const token = { id: "role.surface", value: { ref: { id: "base.light", expectedKind: "token" } } };
  const model = { resolutionOrder: ["scheme", "brand"], contexts: { scheme: "dark", brand: "default" }, axes: [
  ...
  assert.deepEqual(activeTokenReferences!({ ...model, contexts: { scheme: "dark", brand: "custom" } }, token), []);
  assert.equal(activeTokenReferences!({ ...model, contexts: { scheme: "light", brand: "default" } }, token)[0].ref.id, "base.light");
});
