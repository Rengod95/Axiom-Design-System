...
function TokenInspectorForm({ state, controller, locale, creating, model, token, onCreated, onCancel, onSelectComponent, onDirtyChange, externalDirty = false }: Props & { model: FoundationAuthoringProjection; token: FoundationTokenRow | undefined }) {
  ...
  return <div className="foundation-inspector"><header className="inspector-title"><h2>{creating ? t("토큰 만들기", "Create token") : token!.name}</h2><p className="field-hint">{creating ? t("재사용할 값을 정의하세요.", "Define a reusable value.") : `${type} · ${token!.references.length} ${t("참조", "references")}`}</p></header>
    <fieldset disabled={disabled} className="plain-fieldset">
      ...
      <div className="inspector-actions"><Button tone="primary" data-testid="foundation-token-apply" disabled={disabled || !name.trim() || mode === "literal" && !valueValid || mode === "alias" && !(chosenContext ? overrideAlias : alias)} onClick={apply}>{creating ? t("토큰 만들기", "Create token") : t("미리보기에 반영", "Apply to preview")}</Button>{creating && <Button onClick={onCancel}>{t("취소", "Cancel")}</Button>}{dirty && !creating && <Button disabled={disabled} onClick={() => { setDirty(false); changeScope("base"); }}>{t("입력 되돌리기", "Reset form")}</Button>}</div>
    </fieldset>
