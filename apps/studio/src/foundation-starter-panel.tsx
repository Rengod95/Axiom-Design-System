...
function lengthStyle(value: unknown): string | undefined {
  const length = value as JsonObject | undefined;
  return typeof length?.value === "number" && ["px", "rem"].includes(String(length.unit)) ? `${length.value}${length.unit}` : undefined;
}
...
function StarterPreview({ tokens, locale }: { tokens: FoundationStarterToken[]; locale: Locale }) {
  ...
  return <div className="starter-preview" data-testid="starter-template-preview">
    ...
    <div className="starter-preview-body"><div className="starter-preview-specimen" style={{ background: color("surface.canvas"), color: color("text.primary") }}><span style={{ color: color("text.secondary") }}>{t("프로젝트 기본값", "Project defaults")}</span><span className="starter-preview-type" style={{ fontFamily: Array.isArray(type?.fontFamily) ? (type.fontFamily as string[]).join(", ") : undefined, fontSize: lengthStyle(type?.fontSize) }}>{t("의미가 있는 디자인", "Design with meaning")}</span><span className="starter-preview-action" style={{ background: color("action.primary.background"), color: color("action.primary.foreground"), borderRadius: lengthStyle(radius), paddingBlock: lengthStyle(padding) }}>{t("주요 액션", "Primary action")}</span></div>
...
export function StarterOptions({ locale, value, onChange, compact = false }: { locale: Locale; value: FoundationStarterOptions; onChange(value: FoundationStarterOptions): void; compact?: boolean }) {
  ...
  return <div className={`starter-options ${compact ? "compact" : ""}`}>
    ...
    {!compact && <StarterPreview tokens={allTokens} locale={locale} />}
    ...
    <fieldset className="starter-domains starter-required-domains"><legend>{t("모든 템플릿에 기본 포함", "Included in every template")}<span>{FOUNDATION_STARTER_DOMAINS.length}</span></legend><p className="field-hint">{t("색상부터 모션까지, 시스템의 기본 재료와 역할을 함께 준비합니다. 사용하지 않는 도메인은 탐색에서 접어둘 수 있습니다.", "The essential materials and roles of your system, from color to motion. Collapse domains you do not need while browsing.")}</p><div className="starter-required-grid">{FOUNDATION_STARTER_DOMAINS.map(domain => <span key={domain.id}><Icon name={DOMAIN_DETAILS[domain.id]!.icon} size={14} /><strong>{t(...DOMAIN_LABELS[domain.id]!)}</strong><Icon name="check" size={12} /></span>)}</div></fieldset>
  </div>;
}
...
export function FoundationStarterPanel({ locale, disabled, onApply }: { locale: Locale; disabled: boolean; onApply(options: FoundationStarterOptions): boolean }) {
  ...
  return <section className="starter-panel" aria-label={t("기본 토큰 템플릿", "Foundation starter template")}><div className="section-heading"><h2>{t("제품에 맞는 토큰 구조", "A token architecture for your product")}</h2><p>{t("역할과 미리보기를 비교하고, 기본 규격이 갖춰진 구조로 시작하세요.", "Compare roles and previews, then start with a complete foundation.")}</p></div><StarterOptions locale={locale} value={options} onChange={next => { setOptions(next); setApplied(false); }} /><div className="template-preview"><span className="badge">Primitive {tokens.filter(token => token.tier === "primitive").length}</span><span aria-hidden="true">→</span><span className="badge">Semantic {tokens.filter(token => token.tier === "semantic").length}</span>{options.domains.includes("color") && <span className="badge">Light / Dark</span>}</div><div className="form-actions"><Button tone="primary" data-testid="apply-foundation-template" disabled={disabled || !tokens.length} onClick={() => setApplied(onApply({ ...options, domains: FOUNDATION_STARTER_DOMAINS.map(domain => domain.id) }))}>{t("선택한 토큰 추가", "Add selected tokens")}</Button><p className="field-hint">{t("기존 이름·값·테마와 바인딩은 유지합니다. 다른 템플릿을 추가해도 기존 역할은 교체하지 않습니다. 변경 검토에서 저장하세요.", "Existing values, themes and bindings stay intact. Adding another template does not replace existing roles. Save through the shared review.")}</p></div>{applied && <p className="form-success" role="status">{t("토큰이 변경안에 추가되었습니다. 도메인에서 살펴보세요.", "The template joined your proposal. Explore it by domain.")}</p>}</section>;
}
