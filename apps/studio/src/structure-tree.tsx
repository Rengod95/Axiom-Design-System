...
export function StructureTree({ component, category, locale, selectedPart, disabled, onSelect, onEdit }: { component: StudioComponent; category: StudioCategory; locale: Locale; selectedPart: string | null; disabled: boolean; onSelect(partId: string): void; onEdit(edits: StudioComponentEdit[]): boolean }) {
  ...
  const anchor = (part: StudioPart) => component.catalog?.reference ? !part.elementKind : recipe?.parts.some(item => item.role === part.role) ?? true;
  ...
  useEffect(() => {
    ...
    setInsertion(canContainStudioElement(component, category, part.id) ? part.id : containers[0]?.id ?? "");
  ...
  const reorderable = !component.catalog?.reference || siblings.every(part => part.elementKind);
  ...
  const order = (direction: number) => { if (!reorderable || !selected?.parent || at + direction < 0 || at + direction >= siblings.length) return; const ids = siblings.map(part => part.id); [ids[at], ids[at + direction]] = [ids[at + direction]!, ids[at]!]; onEdit([{ kind: "part-order", parentId: selected.parent, childIds: ids }]); };
  ...
  return <div className="structure-editor" data-testid="structure-editor"><ul ref={tree} className="element-tree" role="tree" aria-label={t("컴포넌트 구성요소", "Component structure")}>{render(root, 0)}</ul>
    ...
    {!component.catalog?.reference && needsNormalization && <Button data-testid="structure-normalize" tone="subtle" disabled={disabled} onClick={() => onEdit([{ kind: "structure-normalize" }])}>{t("합성 구조로 정리", "Update compound structure")}</Button>}
    {component.catalog?.reference && !containers.length && <p className="structure-caption">{t("원본 예제에 확인된 삽입 위치가 없습니다. 새 프레임에서 구성하세요.", "This original example has no verified insertion point. Compose inside a new frame.")}</p>}
    {component.catalog && <div className="structure-actions"><label className="structure-insert-target"><span>{t("삽입 위치", "Insert into")}</span><Select data-testid="element-insert-parent" aria-label={t("삽입 위치", "Insert into")} value={insertion} disabled={disabled || !containers.length} onChange={event => setInsertion(event.target.value)}>{containers.map(part => <option key={part.id} value={part.id}>{title(part)}</option>)}</Select></label><div className="structure-add-tools">{(["box", "frame", "text"] as const).map(kind => <Button key={kind} data-testid={`element-add-${kind}`} tone="subtle" icon={kind === "text" ? "type" : kind} disabled={disabled || !containers.some(part => part.id === insertion)} onClick={() => onEdit([{ kind: "element-add", parentId: insertion, element: kind }])}>{kind === "box" ? "Box" : kind === "frame" ? "Frame" : "Text"}</Button>)}</div>
      {selected?.parent && <div className="structure-manage"><IconButton icon="arrow" className="structure-move-up" label={t("앞으로", "Move earlier")} data-testid="part-move-up" disabled={disabled || !reorderable || at <= 0} onClick={() => order(-1)} /><IconButton icon="arrow" className="structure-move-down" label={t("뒤로", "Move later")} data-testid="part-move-down" disabled={disabled || !reorderable || at < 0 || at >= siblings.length - 1} onClick={() => order(1)} />{!anchor(selected) && <Select data-testid="part-parent" aria-label={t("상위 요소 변경", "Move to parent")} value={selected.parent} disabled={disabled} onChange={event => onEdit([{ kind: "part-parent", partId: selected.id, parentId: event.target.value }])}>{containers.filter(part => !isDescendant(part.id, selected.id)).map(part => <option key={part.id} value={part.id}>{title(part)}</option>)}</Select>}<IconButton icon="trash" label={t("요소 삭제", "Delete element")} data-testid="part-delete" disabled={disabled || !removable} onClick={remove} /></div>}
    </div>}
  </div>;
}
