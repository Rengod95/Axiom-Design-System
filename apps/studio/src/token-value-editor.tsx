...
function ValueFields(props: ValueProps) {
  ...
  const measurement = (label: string, measured: JsonObject, measuredType: "dimension" | "duration", measuredPath: string, change: (next: JsonValue) => void) => <div className="dimension-property measurement-property"><Numeric {...props} path={`${measuredPath}/value`} label={label} value={measured.value ?? 0} slider={measuredType === "duration" ? { min: 0, max: measured.unit === "s" ? 2 : 2000, step: measured.unit === "s" ? .01 : 10, variant: "ruler" } : { min: measured.unit === "rem" ? -4 : -64, max: measured.unit === "rem" ? 16 : 256, step: measured.unit === "rem" ? .05 : 1 }} onChange={next => change({ ...measured, value: next })} /><Select aria-label={label === t("값", "Value") ? t("단위", "Unit") : `${label} ${t("단위", "unit")}`} title={t("단위", "Unit")} disabled={disabled} value={String(measured.unit ?? "")} onChange={event => change({ ...measured, unit: event.target.value })}>{(measuredType === "dimension" ? ["px", "rem"] : ["ms", "s"]).map(unit => <option key={unit}>{unit}</option>)}</Select></div>;
