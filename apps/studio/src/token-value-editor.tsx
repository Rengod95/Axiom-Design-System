...
function ValueFields(props: ValueProps) {
  ...
  const measurement = (label: string, measured: JsonObject, measuredType: "dimension" | "duration", measuredPath: string, change: (next: JsonValue) => void) => <div className="dimension-property measurement-property"><Numeric {...props} path={`${measuredPath}/value`} label={label} value={measured.value ?? 0} onChange={next => change({ ...measured, value: next })} /><Select aria-label={label === t("값", "Value") ? t("단위", "Unit") : `${label} ${t("단위", "unit")}`} title={t("단위", "Unit")} disabled={disabled} value={String(measured.unit ?? "")} onChange={event => change({ ...measured, unit: event.target.value })}>{(measuredType === "dimension" ? ["px", "rem"] : ["ms", "s"]).map(unit => <option key={unit}>{unit}</option>)}</Select></div>;
  ...
  if (type === "color") {
    ...
    return <><div className="color-primary">
      ...
      <Numeric {...props} path={`${path}/alpha`} label={t("알파", "Alpha")} value={record.alpha ?? 1} onChange={alpha => onChange({ ...record, alpha })} />
    </div><details className="color-advanced" open={space !== "srgb"}><summary>{t("색 공간과 채널", "Color space and channels")}</summary>
  ...
  if (type === "shadow") {
    ...
    return <><div className="shadow-geometry">{([['offsetX', 'X'], ['offsetY', 'Y'], ['blur', t('흐림', 'Blur')], ['spread', t('퍼짐', 'Spread')]] as const).map(([key, label]) => { const dim = obj(record[key] ?? defaultTokenValue("dimension")); return <div className="dimension-property" key={key}><Numeric {...props} path={`${path}/${key}/value`} label={label} value={dim.value ?? 0} onChange={next => onChange({ ...record, [key]: { ...dim, value: next } })} /><Select aria-label={`${label} ${t("단위", "unit")}`} disabled={disabled} value={String(dim.unit ?? "px")} onChange={event => onChange({ ...record, [key]: { ...dim, unit: event.target.value } })}><option>px</option><option>rem</option></Select></div>; })}</div><label className="check-row"><input type="checkbox" disabled={disabled} checked={record.inset === true} onChange={event => onChange({ ...record, inset: event.target.checked })} />{t("안쪽 그림자", "Inset shadow")}</label>{child("color", "color", t("색", "Color"))}<Button tone="subtle" disabled={disabled} onClick={() => onChange([value])}>{t("그림자 목록으로 전환", "Make a shadow list")}</Button></>;
  }
