...
export function FoundationDomains({ model, locale, onDomain, onTemplate }: { model: FoundationAuthoringProjection; locale: Locale; onDomain(id: string): void; onTemplate(): void }) {
  ...
  return <section className="domain-workspace"><div className="section-heading"><h2>{t("도메인별로 설계하기", "Design by domain")}</h2><p>{t("각 도메인의 기본 스케일, 의미 토큰과 테마 값을 한곳에서 관리하세요.", "Manage each domain’s primitive scales, semantic aliases and themed values together.")}</p></div>
    ...
    <div className="domain-directory">{model.domains.map(domain => {
      ...
      return <button type="button" key={domain.id} className="domain-directory-row" data-testid={`domain-page-${domain.id}`} onClick={() => onDomain(`id:${domain.id}`)}><span className="domain-specimen">{tokens[0] && <TokenSpecimen token={tokens.find(token => token.typeRef.id === "typography") ?? tokens[0]} domain={domain.name.toLowerCase()} />}</span><span><strong>{label ? t(...label) : domain.name}</strong><small>{domain.description || domain.allowedTypes?.join(" · ") || t("모든 유형", "All types")}</small></span><span className="domain-totals"><strong>{tokens.length} {t("토큰", "tokens")}</strong><small>{aliases.length} semantic / alias</small></span><span aria-hidden="true">→</span></button>;
    })}</div>
