...
export function TokenVisual({ type, value, domain = "", name = "", locale = "en", interactive = false }: { type: FoundationTokenType; value: JsonValue | undefined; domain?: string; name?: string; locale?: Locale; interactive?: boolean }) {
  ...
  return <div className={`token-visual token-kind-${type}`} data-visual-type={type} data-visual-domain={domainName}>{content}{isMotion && interactive && <button type="button" className="specimen-replay" onClick={replay} aria-label={copy(locale, "모션 재생", "Replay motion")} title={copy(locale, "최대 10초의 미리보기 · 동작 줄이기 적용", "Preview up to 10 seconds · respects reduced motion")}><Icon name="play" size={14} /></button>}</div>;
}
