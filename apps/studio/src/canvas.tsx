import { ReferenceThemeContext } from "./reference-preview.tsx";
    ...
    <div className="canvas-world" data-testid="canvas-world" style={{ transform: `translate(${view.x}px, ${view.y}px) scale(${view.zoom})` }}>
      {components.map(component => {
        ...
        return <div key={component.id} className={`canvas-frame ${selected ? "is-selected" : ""}`} data-component-frame={component.id} style={{ left: frame.x, top: frame.y, width: frame.width, minHeight: frame.height }}>
          ...
          <div className={`frame-content ${component.catalog?.reference ? "reference-frame-content" : ""}`}><ReferenceThemeContext value={props.previewBackground && /^#[0-9a-f]{6}$/i.test(props.previewBackground) && (parseInt(props.previewBackground.slice(1,3),16) + parseInt(props.previewBackground.slice(3,5),16) + parseInt(props.previewBackground.slice(5,7),16)) < 384 ? "dark" : "light"}><Preview allComponents={props.allComponents ?? components} {...(!disabled && props.onElementEdit ? { onElementEdit: props.onElementEdit } : {})} components={[component]} category={category} mode={mode} selectedPart={selected ? selectedPart : null} onSelect={(id, partId) => onSelect([id], partId)} locale={locale} /></ReferenceThemeContext></div>
