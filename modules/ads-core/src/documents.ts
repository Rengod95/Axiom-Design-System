...
import { CODE, DOCUMENT_KINDS, ID_PATTERN, OPAQUE_FIELDS, RESERVED_IDS, VALIDATION_PROFILES } from "./constants.ts";
...
export function validateReferences(documents: Record<string, DocumentEntry>, projectId: string): Diagnostic[] {
  ...
  for (const entry of Object.values(documents)) if (!VALIDATION_PROFILES.some((profile) => profile === entry.validationProfile)) visit(entry.document, entry.document.id);
  return diagnostics;
}
