...
import { DOMAIN_PROFILE, STRUCTURAL_PROFILE, STUDIO_PROFILE } from "./constants.ts";
import { inspectStudioDocument } from "./studio-validation.ts";
...
export function strongestValidationProfile(...profiles: (ValidationProfile | undefined)[]): ValidationProfile | undefined {
  if (profiles.includes(STUDIO_PROFILE)) return STUDIO_PROFILE;
...
export function inspectProfileDocument(document: JsonValue, profile: ValidationProfile, sourceRef?: string) {
  if (profile === STUDIO_PROFILE) return inspectStudioDocument(document, sourceRef);
