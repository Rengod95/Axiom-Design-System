...
import type { StudioCategory } from "./studio-contracts.ts";
...
export function addStudioElement(component: AdsDocument, designs: AdsDocument[], parentId: string, element: StudioElementKind, id: () => string, placement?: { category: StudioCategory; frame: { x: number; y: number; width: number; height: number } }): void {
  if (!getStudioCatalogRecipe(catalogIdentity(component) ?? "") || !["box", "frame", "text"].includes(element)) throw new Error("Choose a box, frame or text element inside an editable component.");
  if (designs.some(design => sourceElementContent(component, design, parentId) === "none")) throw new Error("Choose a container element. Native controls and text leaves cannot contain authored elements.");
  ...
  const role = `${element}${index}`, added = { id: id(), name: `${element[0]!.toUpperCase()}${element.slice(1)} ${index}`, studioRole: role, studioElement: element, parent: parentId, roleRefs: [], required: false, cardinality: { min: 1, max: 1 }, relationships: [], ...(element === "text" ? { studioText: "Text" } : {}) };
  (component.parts as JsonValue[]).push(added);
  for (const design of designs) {
    const text = element === "text", frame = element === "frame", bounds = placement && placement.category === design.category ? placement.frame : undefined;
    const phrasing = sourceElementContent(component, design, parentId) === "phrasing";
    (design.nodeMappings as JsonValue[]).push({ partRef: added.id, role, element: phrasing ? "span" : text ? "p" : "div" });
    (design.layout as JsonValue[]).push({ targetPartRef: added.id, mode: frame ? "free" : "stack", axis: "vertical", size: { width: bounds ? { mode: "fixed", value: { value: bounds.width, unit: "px" } } : text ? { mode: "hug" } : { mode: "fixed", value: { value: frame ? 240 : 80, unit: "px" } }, height: bounds ? { mode: "fixed", value: { value: bounds.height, unit: "px" } } : text ? { mode: "hug" } : { mode: "fixed", value: { value: frame ? 160 : 80, unit: "px" } } }, position: { x: bounds?.x ?? 24, y: bounds?.y ?? 24 }, gap: { value: 8, unit: "px" }, padding: { value: frame ? 12 : 0, unit: "px" }, minHeight: { value: text ? 24 : 0, unit: "px" }, childOrder: [] });
