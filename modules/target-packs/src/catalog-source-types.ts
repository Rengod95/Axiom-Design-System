...
export function catalogProps(component: StudioComponent, name: string): { declaration: string; destructure: string } {
  ...
  const slots = catalog.slots.map(slot => ({ name: component.parts.find(part => part.id === slot.ownerPartRef)!.role, required: slot.min !== 0 && !(component.instances ?? []).some(instance => instance.ownerPartRef === slot.ownerPartRef) && !component.parts.some(part => part.parent === slot.ownerPartRef && part.elementKind) }));
