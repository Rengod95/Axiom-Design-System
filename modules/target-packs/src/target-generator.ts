...
export function generateTargetPack(project: ProjectSnapshot, options: TargetOptions, digest: TargetDigest): TargetGeneration {
  try {
    ...
    inspectGeneratorProjection(projection);
    ...
    return { valid: true, diagnostics: projection.diagnostics, pack: { files, diagnostics: projection.diagnostics, manifest: {
      ...
      publicApiMap: { ...Object.fromEntries(projection.components.map((component) => [component.id, componentSymbol(component)])), theme: "AxiomThemeProvider", toastHost: "AxiomToastHost", tokens: selected.target === "react" || selected.target === "react-native" ? "tokens" : "AxiomTokens",...(selected.target==="react-native"?{text:"AxiomText"}:{}) },
      capabilities: ["button.native-activation", "card.required-body", "toast.controlled-open", "toast.explicit-host-queue", "toast.bounded-presence", "source.user-owned", "theme.fixed-context"], limitations,
